Now I'll translate the content while preserving all code blocks and technical terms. I'll mark my todos as I complete each step.
The tool requires reading before writing. Since the target file doesn't exist yet, I'll provide the complete translated content directly:

```markdown
name: ultra-finder
description: Motor de análisis de código y descubrimiento de patrones impulsado por IA para exploración automatizada de codebases, detección de vulnerabilidades e identificación de patrones heredados
version: 2.4.1
...
# Ultra Finder - Motor de Análisis con IA

## Propósito

Ultra Finder es un motor de análisis de código inteligente que combina análisis estático con reconocimiento de patrones impulsado por LLM para descubrir vulnerabilidades, anti-patrones, APIs obsoletas, problemas de seguridad y code smells en toda tu codebase.

### Casos de Uso Reales

1. **Pre-Commit Security Audit**: Ejecuta `ultra-finder scan --type security --staged-only` antes de cada commit para detectar SQL injection, XSS y secretos hardcodeados en archivos modificados.

2. **Legacy Modernization**: Detecta y reemplaza patrones obsoletos: `ultra-finder find --pattern "angular.forEach" --replacement "Array.prototype.map" --dry-run` (migra de Angular 1.x a JavaScript moderno).

3. **Performance Bottleneck Hunt**: Encuentra consultas N+1, operaciones de archivo síncronas y llamadas bloqueantes: `ultra-finder analyze --profile performance --path src/ --since "2 weeks ago"`.

4. **API Migration Assistant**: Identifica todos los usos de endpoints obsoletos: `ultra-finder scan --type api-deprecation --target v2.0.0 --report json > migration-plan.json`.

5. **Code Ownership Mapping**: Descubre quién es dueño de qué módulos analizando historial de commits y complejidad: `ultra-finder analyze --scope ownership --output ownership.md`.

6. **Technical Debt Quantification**: Mide complejidad ciclomática, duplicación y brechas de cobertura de tests: `ultra-finder audit --debt --threshold high --export debt-report.xlsx`.
...
### Example 2: Detectando Async Anti-Patterns
...
- YAML description field translated to Spanish
- "Scope" → "## Alcance"
- "Detailed Work Process" → "## Proceso de Trabajo Detallado"
- "Golden Rules" → "## Reglas de Oro"
- "Troubleshooting" → "## Solución de Problemas"
- Other headers translated naturally (Propósito, Casos de Uso Reales, Ejemplos, etc.)
- All code blocks preserved in English
- Technical terms (CLI, JSON, YAML, API, LLM, SQL, XSS, etc.) kept in English
- Commands, file names, and code snippets unchanged